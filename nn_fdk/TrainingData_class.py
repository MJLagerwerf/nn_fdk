...
class TrainingData(object):
    ...
    def __setupIDX(self, size):
        ...
        ym, xm = np.ogrid[-(size-1.) / 2.:(size-1.) / 2.:complex(0, size),
                         -(size-1.) / 2.:(size-1.) / 2.:complex(0, size)]
        bnd = (size) ** 2 / 4
        self.mask = xm ** 2 + ym ** 2 <= bnd
        x, y = np.where(self.mask is True)
        self.idx = zip(x, y)
    ...
    def __init__(self, data, nPoints, network, blockSize=10000):
        ...
        self.fn = tmpFl[1]
        os.close(tmpFl[0])
        pickArray = np.histogram(np.floor(data.nImages*np.random.rand(nPoints)),
                                 data.nImages, (0, data.nImages))[0]
        ...
        for i in range(len(data)):
            ...
            if nInBlock + nToPick < blockSize:
                ...
                if nToWrite > 0:
                    outData[nInBlock:blockSize, :] = curData[0:nToWrite,
                                                       :].copy()
            ...
            sys.stdout.write('\r[%s>%s] %d%% %s' % ('-'*nTicksDone,
                             ' '*(60-nTicksDone), 100*percDone, 50*' '))
            sys.stdout.flush()
        if nInBlock > 0:
...
class MATTrainingData(TrainingData):
    ...
    def getDataBlock(self, i):
        if i in self.normalized:
            return np.asarray(self.normalized[i])

        if not self.tileM is None:
            data = sio.loadmat(self.fls[i])[self.dataname]
            data[:, 0:self.nPar] = 2 * (data[:, 0:self.nPar] - self.tileM) / \
                                    self.maxmin - 1
            data[:, self.nPar] = 0.25 + (data[:, self.nPar] - self.minIn) / \
                                    (2 * (self.maxIn - self.minIn))
            rand_name = ''.join(random.choices(string.ascii_uppercase +
                                               string.digits, k=10))
            self.fn[i] = '/export/scratch2/lagerwer/data/data_dump/' + \
                            rand_name + '.cache'

            self.normalized[i] = np.memmap(self.fn[i], mode='w+',
                           dtype='float64', shape=data.shape)
            self.normalized[i][:] = data[:]
            return data
        else:
    ...
    def normalizeData(self, minL, maxL, minIn, maxIn):
        ...
        self.minIn = minIn
        self.maxIn = maxIn
        self.normalized = {} #np.zeros(np.shape(self.fls)[0], dtype=bool)
        self.fn = {}

    def __init__(self, fls, dataname='mat'):
        # Create a datadump map
        if not os.path.exists('/export/scratch2/lagerwer/data/data_dump/'):
            os.makedirs('/export/scratch2/lagerwer/data/data_dump/')
        self.fls = fls
